...
class ApiService {
  ...
  static Future<bool> startNewProcedureAfterCompletion() async {
    try {
      ...
      final response = await http.post(url, headers: headers).timeout(_slowEndpointTimeout);
      if (response.statusCode == 200) {
        return true;
      } else {
        print('Start new procedure failed: ${response.statusCode} → ${response.body}');
        return false;
      }
    } catch (e) {
  ...
  static Future<Map<String, dynamic>?> getUserDetails() async {
    ...
    for (int attempt = 0; attempt <= retries; attempt++) {
      try {
        ...
        final response =
            await http.get(Uri.parse('$baseUrl/patients/me'), headers: headers).timeout(const Duration(seconds: 25));
  ...
  static Future<List<Map<String, dynamic>>> getPatientsByDoctor(String doctor) async {
    try {
      ...
      final headers = await getDoctorAuthHeaders();
      // Use the episode-based feed so completed procedures show up as separate rows
      // (matches the old dashboard behavior: history visible, not just current snapshot).
