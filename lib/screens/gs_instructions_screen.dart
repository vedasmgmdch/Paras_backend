...
class _GSInstructionsScreenState extends State<GSInstructionsScreen>
    with InstructionSnapshotHelper<GSInstructionsScreen> {
  ...
  String _generalChecklistKey(DateTime date) => "gs_dos_${date.year}_${date.month}_${date.day}";
  String _specificChecklistKey(DateTime date) => "gs_specific_${date.year}_${date.month}_${date.day}";
  ...
  void initState() {
    ...
    if (_dosChecked.length != gsDos.length) {
      _dosChecked = List.filled(gsDos.length, false);
  ...
  void _updateSpecificChecklist(int idx, bool value) {
    ...
    _logInstructionChange(
      logDate: selectedDate,
      group: 'specific',
      instructionEn: gsSpecificInstructions[idx]['en'] ?? '',
      followed: value,
    );
  }
...
"""
  ...
  Widget build(BuildContext context) {
    ...
    if (treatment != null && treatment.isNotEmpty) {
      ...
      // fallback to default
      title = selectedLang == 'en' ? "Instructions (Gum Surgery)" : "सूचना (गम सर्जरी)";
    }
                      ...
                      ...List.generate(
                        ...
                        (i) => Padding(
                          ...
                          child: CheckboxListTile(
                            ...
                            controlAffinity: ListTileControlAffinity.leading,
                            dense: true,
                            title: Text(gsSpecificInstructions[i][selectedLang]!, style: const TextStyle(fontSize: 15)),
