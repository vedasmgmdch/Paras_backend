...
class TCInstructionsScreen extends StatefulWidget {
  final DateTime? date;

  const TCInstructionsScreen({super.key, this.date});
  @override
  State<TCInstructionsScreen> createState() => _TCInstructionsScreenState();
}

class _TCInstructionsScreenState extends State<TCInstructionsScreen>
    with InstructionSnapshotHelper<TCInstructionsScreen> {
  ...
  static const List<Map<String, String>> tcDos = [
  ...
  static const List<Map<String, String>> tcDonts = [
    {
  ...
  static const List<Map<String, String>> tcSpecificInstructions = [
    {
  ...
  String _generalChecklistKey(DateTime date) => "tc_dos_${date.year}_${date.month}_${date.day}";
  String _specificChecklistKey(DateTime date) => "tc_specific_${date.year}_${date.month}_${date.day}";
  ...
  void initState() {
    ...
    if (_dosChecked.length != tcDos.length) {
      _dosChecked = List.filled(tcDos.length, false);
    ...
    if (_specificChecked.length != tcSpecificInstructions.length) {
      _specificChecked = List.filled(tcSpecificInstructions.length, false);
      WidgetsBinding.instance.addPostFrameCallback((_) {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) async {
      ...
      void hydrateFromAppState() {
        final hydratedGeneral = appState.buildFollowedChecklistForDay(
          ...
          type: 'general',
          length: tcDos.length,
          instructionTextForIndex: (i) => tcDos[i]['en'] ?? '',
          username: appState.username,
          treatment: appState.treatment,
          subtype: appState.treatmentSubtype,
        );
        final hydratedSpecific = appState.buildFollowedChecklistForDay(
          day: selectedDate,
          type: 'specific',
          length: tcSpecificInstructions.length,
          instructionTextForIndex: (i) => tcSpecificInstructions[i]['en'] ?? '',
          username: appState.username,
          treatment: appState.treatment,
          subtype: appState.treatmentSubtype,
        );
  ...
  void _updateSpecificChecklist(int idx, bool value) {
    ...
    _logInstructionChange(
      logDate: selectedDate,
      group: 'specific',
      instructionEn: tcSpecificInstructions[idx]['en'] ?? '',
      followed: value,
    );
  }
  ...
  void _logInstructionStatusIfNeeded() {
    ...
    for (int i = 0; i < tcDos.length; i++) {
      if (!_dosChecked[i]) notFollowedGeneral.add(tcDos[i][selectedLang]!);
    }

    final List<String> notFollowedSpecific = [];
    for (int i = 0; i < tcSpecificInstructions.length; i++) {
      if (!_specificChecked[i]) notFollowedSpecific.add(tcSpecificInstructions[i][selectedLang]!);
    }
    ...
    final String log = """
[Teeth Cleaning] $dateStr (Day $currentDay)
...
"""
  ...
  Widget build(BuildContext context) {
    ...
    if (treatment != null && treatment.isNotEmpty) {
      ...
      title = selectedLang == 'en' ? "Instructions (Teeth Cleaning)" : "सूचना (दात स्वच्छ करणे)";
    }
                      ...
                      ...List.generate(
                        ...
                        (i) => Padding(
                          ...
                          child: CheckboxListTile(
                            ...
                            controlAffinity: ListTileControlAffinity.leading,
                            dense: true,
                            title: Text(tcSpecificInstructions[i][selectedLang]!, style: const TextStyle(fontSize: 15)),
