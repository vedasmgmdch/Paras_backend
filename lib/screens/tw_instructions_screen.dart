...
class TWInstructionsScreen extends StatefulWidget {
  final DateTime? date;

  const TWInstructionsScreen({super.key, this.date});

  @override
  State<TWInstructionsScreen> createState() => _TWInstructionsScreenState();
}

class _TWInstructionsScreenState extends State<TWInstructionsScreen>
  ...
  static const List<Map<String, String>> twDos = [
  ...
  static const List<Map<String, String>> twDonts = [
    {
  ...
  static const List<Map<String, String>> twSpecificInstructions = [
    {
  ...
  String _generalChecklistKey(DateTime date) => "tw_dos_${date.year}_${date.month}_${date.day}";
  String _specificChecklistKey(DateTime date) => "tw_specific_${date.year}_${date.month}_${date.day}";
  ...
  void initState() {
    ...
    final bool dosLenChanged = _dosChecked.length != twDos.length;
    if (dosLenChanged) {
      _dosChecked = List.filled(twDos.length, false);
    ...
    final bool specLenChanged = _specificChecked.length != twSpecificInstructions.length;
    if (specLenChanged) {
      _specificChecked = List.filled(twSpecificInstructions.length, false);
      WidgetsBinding.instance.addPostFrameCallback((_) {
  ...
  void _updateSpecificChecklist(int idx, bool value) {
    ...
    _logInstructionChange(
      logDate: selectedDate,
      group: 'specific',
      instructionEn: twSpecificInstructions[idx]['en'] ?? '',
      followed: value,
    );
  }
  ...
  void _logInstructionStatusIfNeeded() {
    ...
    for (int i = 0; i < twDos.length; i++) {
      if (!_dosChecked[i]) notFollowedGeneral.add(twDos[i][selectedLang]!);
    }

    final List<String> notFollowedSpecific = [];
    for (int i = 0; i < twSpecificInstructions.length; i++) {
      if (!_specificChecked[i]) notFollowedSpecific.add(twSpecificInstructions[i][selectedLang]!);
    }
...
"""
  ...
  Widget build(BuildContext context) {
    ...
    if (treatment != null && treatment.isNotEmpty) {
      ...
      title = selectedLang == 'en' ? "Instructions (Tooth Whitening)" : "सूचना (दात पांढरे करणे)";
    }
                      ...
                      ...List.generate(
                        ...
                        (i) => Padding(
                          ...
                          child: CheckboxListTile(
                            ...
                            controlAffinity: ListTileControlAffinity.leading,
                            dense: true,
                            title: Text(twSpecificInstructions[i][selectedLang]!, style: const TextStyle(fontSize: 15)),
