...
void main() async {
  ...
  // Remove any stale schedules left from older versions, then schedule from server list once token is available
  try {
    await NotificationService.cancelAllPending();
  } catch (e) {
    print('Startup cancelAllPending failed: $e');
  }
  // Initialize hybrid reminders (local + server fallback) after base notification init.
  // This will attach ack callback and later we'll sync after user token available.
  // HybridRemindersService removed.

  final appState = AppState();
  await appState.syncTokenFromPrefs();       // <-- Ensure token is loaded!
  // Fetch server UTC now at startup so day calculations use server time
  try {
  ...
  // After token is ready, fetch server reminders and schedule locally to ensure schedules exist even if user doesn't open Reminders screen yet.
  try {
  ...
  runApp(
    ChangeNotifierProvider(create: (_) => appState, child: const ToothCareGuideApp()),
  );

  // Push registration handled above via PushService; any future hybrid sync would go here.
}
